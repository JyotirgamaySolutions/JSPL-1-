...
class MatchingGameScene extends Phaser.Scene {
    constructor() {
        ...
        this.correctAnswers = ['Honey_bee', 'Wasp', 'Moth', 'Flies'  ]; // Define correct answers
        this.correctMatches = new Set(); // Track matched correct answers
    }

    preload() {
        // Load images for the game
        this.load.image('rectangleCard6', 'assets/6.png'); // Updated to 2.png
...
createRectangleCard(width) {
    ...
    const card = this.add.image(width / 2, yPos, 'rectangleCard6')
        .setOrigin(0.5)
        .setInteractive();
    ...
    card.on('pointerdown', () => {
        this.displayPopup(card.x, card.y, 
            'India is the largest cotton producer globally, accounting for 23% of total global cotton production.'
        );
    });
...
const isLaptop = window.innerHeight < 900;
    ...
    checkIncorrectAttempts() {
        ...
        if (this.incorrectAttempts >= this.maxIncorrectAttempts && this.correctMatches.size < this.correctAnswers.length) {
            this.displayPopup(this.sys.game.config.width / 2, this.sys.game.config.height / 2, 'The correct are Wasp, Flies, Moths, and Honey Bees' );
        }
    }
    ...
    addLeftArrow(width, height) {
        ...
        leftArrow.on('pointerdown', () => {
            this.scene.start('lvl5'); // Adjust to your actual previous scene key
        });
    }
}
