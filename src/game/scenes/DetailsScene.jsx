...
class MatchingGameScene extends Phaser.Scene {
    constructor() {
        ...
        this.correctAnswers = ['Bumblebee', 'Solitary_bee']; // Define correct answers
        this.correctMatches = new Set(); // Track matched correct answers
    }

    preload() {
        // Load images for the game
        this.load.image('rectangleCard3', 'assets/3.png'); // Updated to 2.png
...
createRectangleCard(width) {
    ...
    const card = this.add.image(width / 2, yPos, 'rectangleCard3')
        .setOrigin(0.5)
        .setInteractive();
    ...
    card.on('pointerdown', () => {
        this.displayPopup(card.x, card.y, 
            'In fiscal year 2023, the export of potatoes from India was valued at over seven billion Indian rupees.', 
        );
    });
...
const isLaptop = window.innerHeight < 900;
    ...
    checkIncorrectAttempts() {
        ...
        if (this.incorrectAttempts >= this.maxIncorrectAttempts && this.correctMatches.size < this.correctAnswers.length) {
            this.displayPopup(this.sys.game.config.width / 2, this.sys.game.config.height / 2, 'The correct answers are Bumblebee and Solitary Bee.');
        }
    }
...
displayPopup(x, y, message, hasCloseButton = false) {
    ...
    addLeftArrow(width, height) {
        ...
        leftArrow.on('pointerdown', () => {
            this.scene.start('lvl2'); // Adjust to your actual previous scene key
        });
    }
}
