...
class MatchingGameScene extends Phaser.Scene {
    constructor() {
        ...
        this.correctAnswers = ['Honey_bee', 'Stingless_bees'  ]; // Define correct answers
        this.correctMatches = new Set(); // Track matched correct answers
    }

    preload() {
        // Load images for the game
        this.load.image('rectangleCard10', 'assets/10.png'); // Updated to 2.png
...
createRectangleCard(width) {
    ...
    const card = this.add.image(width / 2, yPos, 'rectangleCard10')
        .setOrigin(0.5)
        .setInteractive();
    ...
    card.on('pointerdown', () => {
        this.displayPopup(card.x, card.y, 
            'During 2022-23, India’s coconut products exports are valued at Rs. 3,555 crore.',
        );
    });
...
const isLaptop = window.innerHeight < 900;
    ...
    checkIncorrectAttempts() {
        ...
        if (this.incorrectAttempts >= this.maxIncorrectAttempts && this.correctMatches.size < this.correctAnswers.length) {
            this.displayPopup(this.sys.game.config.width / 2, this.sys.game.config.height / 2, 'The correct answers are Honey bee, stingless bee  ');
        }
    }
  ...
  displayPopup(x, y, message, hasCloseButton = false) {
    ...
    addLeftArrow(width, height) {
        ...
        leftArrow.on('pointerdown', () => {
            this.scene.start('lvl9'); // Adjust to your actual previous scene key
        });
    }
}
