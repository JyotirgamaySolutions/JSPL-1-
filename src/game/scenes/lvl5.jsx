...
class MatchingGameScene extends Phaser.Scene {
    constructor() {
        ...
        this.correctAnswers = ['Solitary_bee','Honey_bee'  ]; // Define correct answers
        this.correctMatches = new Set(); // Track matched correct answers
    }

    preload() {
        // Load images for the game
        this.load.image('rectangleCard5', 'assets/5.png'); // Updated to 2.png
...
createRectangleCard(width) {
    ...
    const card = this.add.image(width / 2, yPos, 'rectangleCard5')
        .setOrigin(0.5)
        .setInteractive();
    ...
    card.on('pointerdown', () => {
        this.displayPopup(card.x, card.y, 
            'India is the world’s largest spice producer. It is also the largest consumer and exporter of spices. Cardamom is one of the most produced and exported spices.'
        );
    });
...
const isLaptop = window.innerHeight < 900;
    ...
    checkIncorrectAttempts() {
        ...
        if (this.incorrectAttempts >= this.maxIncorrectAttempts && this.correctMatches.size < this.correctAnswers.length) {
            this.displayPopup(this.sys.game.config.width / 2, this.sys.game.config.height / 2, 'The correct answers are Honey bee, Solitary bee' );
        }
    }
    ...
    addLeftArrow(width, height) {
        ...
        leftArrow.on('pointerdown', () => {
            this.scene.start('lvl4'); // Adjust to your actual previous scene key
        });
    }
}
