...
class MatchingGameScene extends Phaser.Scene {
    constructor() {
        ...
        this.correctAnswers = ['Bumblebee', 'Solitary_bee','Stingless_bees','Honey_bee', 'Butterfly', 'Flies'  ]; // Define correct answers
        this.correctMatches = new Set(); // Track matched correct answers
    }

    preload() {
        // Load images for the game
        this.load.image('rectangleCard4', 'assets/4.png'); // Updated to 2.png
...
createRectangleCard(width) {
    ...
    const card = this.add.image(width / 2, yPos, 'rectangleCard4')
        .setOrigin(0.5)
        .setInteractive();
...
const isLaptop = window.innerHeight < 900;
    ...
    checkIncorrectAttempts() {
        ...
        if (this.incorrectAttempts >= this.maxIncorrectAttempts && this.correctMatches.size < this.correctAnswers.length) {
            this.displayPopup(this.sys.game.config.width / 2, this.sys.game.config.height / 2, 'The correct answers for cashew are Honey bees, stingless bees, bumblebees, solitary bees, butterflies, flies,');
        }
    }
    ...
    popupBackground.fillRoundedRect(x - 220, y - 100, 440, 200, 40);  // Adjust size and position
    popupBackground.lineStyle(4, 0xFFD700);  // Add a golden border
    popupBackground.strokeRoundedRect(x - 220, y - 100, 440, 200, 40);  // Draw the border
    ...
    addLeftArrow(width, height) {
        ...
        leftArrow.on('pointerdown', () => {
            this.scene.start('DetailsScene'); // Adjust to your actual previous scene key
        });
    }
}
