...
class MatchingGameScene extends Phaser.Scene {
    constructor() {
        ...
        this.correctAnswers = ['Honey_bee', 'Solitary_bee'  ]; // Define correct answers
        this.correctMatches = new Set(); // Track matched correct answers
    }

    preload() {
        // Load images for the game
        this.load.image('rectangleCard8', 'assets/8.png'); // Updated to 2.png
...
createRectangleCard(width) {
    ...
    const card = this.add.image(width / 2, yPos, 'rectangleCard8')
        .setOrigin(0.5)
        .setInteractive();
    ...
    card.on('pointerdown', () => {
        this.displayPopup(card.x, card.y, 
            'The mustard seed production stood at 8.80 million tons in the 2022-23 season.'
        );
    });
...
const isLaptop = window.innerHeight < 900;
    ...
    checkIncorrectAttempts() {
        ...
        if (this.incorrectAttempts >= this.maxIncorrectAttempts && this.correctMatches.size < this.correctAnswers.length) {
            this.displayPopup(this.sys.game.config.width / 2, this.sys.game.config.height / 2, 'The correct answers are Honey bees, solitary bee');
        }
    }
  ...
  displayPopup(x, y, message, hasCloseButton = false) {
    ...
    addLeftArrow(width, height) {
        ...
        leftArrow.on('pointerdown', () => {
            this.scene.start('lvl7'); // Adjust to your actual previous scene key
        });
    }
}
